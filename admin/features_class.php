<?php 
	...
	function cadastroFeatures($dados)
	{
		...
		$sql = "INSERT INTO features(icone, nome, numero, descricao) VALUES (
						'".$dados['icone']."',
						'".$dados['nome']."',
						'".$dados['numero']."',
						'".$dados['descricao']."')";
	...
	function buscaFeatures($dados = array())
	{
		...
		$sql = "SELECT $colsSql $buscaMax FROM features as C 
				WHERE 1  $buscaId  $buscaNome $buscaNumero $buscaOrdem $orderBy $buscaLimit ";
		...
		while ($r = mysqli_fetch_assoc($query)){
            ...
            if (!array_key_exists('totalRecords', $dados)) {
                ...
                $StringIcone > 4 ? $r['icone_caminho'] = "<img src='files/features/" . $r['icone'] . "' class='icone' />" : $r['icone_caminho'] = '<i class="fa fa-' . $icones_Edit['nome'] . ' fa-2x icone"></i>';
                $StringIcone > 4 ? $r['icone_front'] = "<img src='admin/files/features/" . $r['icone'] . "' class='mr-2 icone' />" : $r['icone_front'] = '<i class="fa fa-' . $icones_Edit['nome'] . ' fa-lg mr-2 icone" style="color:#2460a7; font-size: 53px"></i>';
